...
void RC_Data_t::check_validity()
{
    ...
    reboot_cmd <= 1.1)
  {
    ...
    RCLCPP_ERROR(
      node_->get_logger(), "RC data validity check fail. mode=%f, gear=%f, reboot_cmd=%f", mode, gear,
      reboot_cmd);
  }
}
...
// #define VEL_IN_BODY
  ...
  Eigen::Quaternion<double> wRb_q(msg.pose.pose.orientation.w, msg.pose.pose.orientation.x,
  ...
  static int count = 0;
  if (count++ % 500 == 0) {
    RCLCPP_WARN(node_->get_logger(), "VEL_IN_BODY!!!");
  }
#endif
