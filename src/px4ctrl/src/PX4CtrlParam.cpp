...
void Parameter_t::config_from_ros_handle(const std::shared_ptr<rclcpp::Node> & node)
{
  ...
  read_essential_param(node, "gain.Kvd0", gain.Kvd0);   // X轴速度微分增益
  read_essential_param(node, "gain.Kvd1", gain.Kvd1);   // Y轴速度微分增益
  read_essential_param(node, "gain.Kvd2", gain.Kvd2);   // Z轴速度微分增益
  ...
  if (takeoff_land.enable_auto_arm && !takeoff_land.enable) {
    ...
    RCLCPP_ERROR(
      node->get_logger(), "\"enable_auto_arm\" is only allowd with \"auto_takeoff_land\" enabled.");
  }
  if (takeoff_land.no_RC && (!takeoff_land.enable_auto_arm || !takeoff_land.enable) ) {
    takeoff_land.no_RC = false;
    RCLCPP_ERROR(
      node->get_logger(),
      "\"no_RC\" is only allowd with both \"auto_takeoff_land\" and \"enable_auto_arm\" enabled.");
  }

  if (thr_map.print_val) {
    RCLCPP_WARN(
      node->get_logger(), "You should disable \"print_value\" if you are in regular usage.");
  }
}
