...
PX4CtrlFSM::PX4CtrlFSM(
  Parameter_t & param_, LinearControl & controller_,
    ...
    case MANUAL_CTRL:
      {
        if (rc_data.enter_hover_mode) {       // 尝试跳转到AUTO_HOVER
          ...
          if (cmd_is_received(now_time)) {
            ...
            RCLCPP_ERROR(
              node_->get_logger(),
              "Reject AUTO_HOVER(L2). You are sending commands before toggling into AUTO_HOVER, which is not allowed. Stop sending commands now!");
            break;
          }
          if (odom_data.v.norm() > 3.0) {
            // 拒绝AUTO_HOVER(L2),因为里程计数据表明无人机速度大于3m/s,这可能表明定位模块出现了问题!
            RCLCPP_ERROR(
              node_->get_logger(), "Reject AUTO_HOVER(L2). Odom_Vel=%fm/s, which seems that the locolization module goes wrong!",
              odom_data.v.norm());
            break;
          }
          if (set_offboard_flag || toggle_offboard_mode(true)) {// 切换到offboard模式
            if (state_data.current_state.mode == "OFFBOARD") {
              ...
              RCLCPP_INFO(
                node_->get_logger(), "\033[32m[px4ctrl] MANUAL_CTRL(L1) --> AUTO_HOVER(L2)\033[0m");
            } else {
              RCLCPP_DEBUG(node_->get_logger(), "Waiting for OFFBOARD mode confirmation.");
              break;
            }
          } else {
            RCLCPP_ERROR(node_->get_logger(), "Reject AUTO_HOVER. Failed to toggle offboard mode!");
            break;
          }
        } else if (param.takeoff_land.enable && takeoff_land_data.triggered &&
          takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::msg::TakeoffLand::TAKEOFF)                                                                            // Try to jump to AUTO_TAKEOFF

        {
          ...
          if (cmd_is_received(now_time)) {              // 拒绝AUTO_TAKEOFF,因为你正在进入AUTO_TAKEOFF之前发送指令,这是不允许的,请立即停止发送指令!
            ...
            RCLCPP_ERROR(
              node_->get_logger(),
              "Reject AUTO_TAKEOFF. You are sending commands before toggling into AUTO_TAKEOFF, which is not allowed. Stop sending commands now!");
            break;
          }
          if (odom_data.v.norm() > 0.1) {               // 拒绝AUTO_TAKEOFF,因为里程计数据表明无人机速度大于0.1m/s,这是不允许的!
            takeoff_land_data.triggered = false;
            RCLCPP_ERROR(
              node_->get_logger(), "Reject AUTO_TAKEOFF. Odom_Vel=%fm/s, non-static takeoff is not allowed!",
              odom_data.v.norm());
            break;
          }
          if (!get_landed()) {                                          // 拒绝AUTO_TAKEOFF,因为起飞降落检测器说无人机现在没有降落!
            ...
            RCLCPP_ERROR(
              node_->get_logger(),
              "Reject AUTO_TAKEOFF. land detector says that the drone is not landed now!");
            break;
          }
          if (rc_is_received(now_time)) {               // 检查遥控器是否连接
            if (!rc_data.is_hover_mode || !rc_data.is_command_mode || !rc_data.check_centered()) {
              ...
              RCLCPP_ERROR(
                node_->get_logger(),
                "Reject AUTO_TAKEOFF. If you have your RC connected, keep its switches at \"auto hover\" and \"command control\" states, and all sticks at the center, then takeoff again.");
              while (rclcpp::ok()) {
                ...
                if (rc_data.is_hover_mode && rc_data.is_command_mode && rc_data.check_centered()) {
                  ...
                  takeoff_land_data.triggered = false;
                  RCLCPP_INFO(
                    node_->get_logger(), "\033[32m[px4ctrl] OK, you can takeoff again.\033[0m");
                  break;
                }
              }
              break;
          ...
          if (set_offboard_flag || toggle_offboard_mode(true)) {
            if (state_data.current_state.mode == "OFFBOARD") {
              ...
              RCLCPP_INFO(
                node_->get_logger(), "\033[32m[px4ctrl] MANUAL_CTRL(L1) --> AUTO_TAKEOFF\033[0m");
            } else {
              RCLCPP_DEBUG(node_->get_logger(), "Waiting for OFFBOARD mode confirmation.");
              break;
            }
            ...
            RCLCPP_ERROR(
              node_->get_logger(),
              "Reject AUTO_TAKEOFF. Failed to toggle offboard mode!");
            break;
          }
        }
    ...
    case AUTO_HOVER:
      {
        ...
        // 如果遥控器进入指令模式并且收到指令数据
          ...
          if (state_data.current_state.mode == "OFFBOARD") {
            ...
            RCLCPP_INFO(
              node_->get_logger(), "\033[32m[px4ctrl] AUTO_HOVER(L2) --> CMD_CTRL(L3)\033[0m");
          }
        }
          ...
          takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::msg::TakeoffLand::LAND)
        {
            ...
            (takeoff_land.delay_trigger.first && now_time > takeoff_land.delay_trigger.second))
          {
            ...
            RCLCPP_INFO(
              node_->get_logger(), "\033[32m[px4ctrl] TRIGGER sent, allow user command.\033[0m");
          }
    ...
    case CMD_CTRL:
      {
        ...
        {
          RCLCPP_ERROR(
            node_->get_logger(), "Reject AUTO_LAND, which must be triggered in AUTO_HOVER. \
    ...
    case AUTO_TAKEOFF:
      {
          ...
          AutoTakeoffLand_t::MOTORS_SPEEDUP_TIME)                                                                          // Wait for several seconds to warn prople.
        {
          ...
          set_hov_with_odom();
          RCLCPP_INFO(
            node_->get_logger(), "\033[32m[px4ctrl] AUTO_TAKEOFF --> AUTO_HOVER(L2)\033[0m");
    ...
    case AUTO_LAND:
      {
        if (!rc_data.is_hover_mode || !odom_is_received(now_time)) {
          ...
          if (print_once_flag) {
            RCLCPP_INFO(
              node_->get_logger(),
              "\033[32m[px4ctrl] Wait for abount 10s to let the drone arm.\033[0m");
            print_once_flag = false;
          }

          if (extended_state_data.current_extended_state.landed_state ==
            mavros_msgs::msg::ExtendedState::LANDED_STATE_ON_GROUND)                                                                            // PX4 allows disarm after this
          {
            ...
            if (now_time.seconds() - last_trial_time > 1.0) {
              if (toggle_arm_disarm(false)) {                         // disarm
                ...
                RCLCPP_INFO(
                  node_->get_logger(), "\033[32m[px4ctrl] AUTO_LAND --> MANUAL_CTRL(L1)\033[0m");
              }
...
bool PX4CtrlFSM::toggle_arm_disarm(bool arm)
{
  ...
  if (!result->success) {
    RCLCPP_ERROR(
      node_->get_logger(), "\033[31m%s rejected by PX4! Result code: %d\033[0m",
      arm ? "ARM" : "DISARM",
      result->result);
    return false;
  }
